...
public class GallerySnapHelper extends LinearSnapHelper {
    ...
    int[] calculateDistanceToFinalSnap(@NonNull RecyclerView.LayoutManager layoutManager, @NonNull View targetView) {
        ...
        if (layoutManager.canScrollVertically()) {
            out[1] = this.distanceToCenter(layoutManager, targetView, getVerticalHelper(layoutManager));
        }
    ...
    public int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX, int velocityY) {
        ...
        hDeltaJump *= getSpanCount(layoutManager);
//        if (hDeltaJump == 0) {
//            return RecyclerView.NO_POSITION;
//        }
    ...
    private View findStartView(RecyclerView.LayoutManager layoutManager, OrientationHelper helper) {
        if (layoutManager instanceof LinearLayoutManager) {
            ...
            if (helper.getDecoratedEnd(firstChildView) >= helper.getDecoratedMeasurement(firstChildView) / 2 && helper.getDecoratedEnd(firstChildView) > 0) {
                ...
                return layoutManager.findViewByPosition(firstChildPosition + getSpanCount(layoutManager));
            }
        } else {
            return null;
        }
    ...
    private float computeDistancePerChild(RecyclerView.LayoutManager layoutManager,
        ...
        return 1f * distance / ((maxPos - minPos) + getSpanCount(layoutManager));
    }
