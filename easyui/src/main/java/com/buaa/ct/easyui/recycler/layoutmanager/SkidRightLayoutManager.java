...
public class SkidRightLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void fill(RecyclerView.Recycler recycler) {
        ...
        for (int i = bottomItemPosition - 1, j = 1;
             i >= 0; i--, j++) {
            double maxOffset = baseOffsetSpace / 2.0 * Math.pow(mScale, j);
            ...
            if (isOutOfSpace) {
                ...
                info.setLayoutPercent(info.getTop() * 1f / space);
    ...
    public static class SkidRightSnapHelper extends SnapHelper {
