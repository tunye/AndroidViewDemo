...
import com.buaa.ct.easyui.recycler.layoutmanager.entity.ItemViewInfo;
...
public class EchelonLayoutManager extends RecyclerView.LayoutManager {
    ...
    private void layoutChild(RecyclerView.Recycler recycler) {
        ...
        int bottomItemPosition = (int) Math.floor(mScrollOffset * 1.0 / mItemViewHeight);
        ...
        List<ItemViewInfo> layoutInfos = new ArrayList<>();
        for (int i = bottomItemPosition - 1, j = 1; i >= 0; i--, j++) {
            double maxOffset = (getVerticalSpace() - mItemViewHeight) / 2.0 * Math.pow(0.8, j);
            ...
            ItemViewInfo info = new ItemViewInfo(start, scaleXY, offsetPercentRelativeToItemView, layoutPercent);
            layoutInfos.add(0, info);
            remainSpace = (int) (remainSpace - maxOffset);
            if (remainSpace <= 0) {
                info.setTop((int) (remainSpace + maxOffset));
                info.setPositionOffset(0);
                info.setLayoutPercent(info.getTop() * 1f / getVerticalSpace());
        ...
        if (bottomItemPosition < mItemCount) {
            ...
            layoutInfos.add(new ItemViewInfo(start, 1.0f, bottomItemVisibleHeight * 1.0f / mItemViewHeight, start * 1.0f / getVerticalSpace(), true));
        } else {
