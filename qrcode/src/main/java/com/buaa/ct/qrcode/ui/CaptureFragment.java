...
import com.buaa.ct.qrcode.camera.CameraManager;
import com.buaa.ct.qrcode.decoding.CaptureViewHandler;
import com.buaa.ct.qrcode.decoding.InactivityTimer;
import com.buaa.ct.qrcode.util.QRCodeAnalyzeUtils;
import com.buaa.ct.qrcode.view.ViewfinderView;
...
public class CaptureFragment extends Fragment implements ICaptureView, SurfaceHolder.Callback {
    ...
    public static CaptureFragment newInstance(int layoutId, boolean isRepeated, long scanInterval) {
        ...
        bundle.putInt(QRCode.KEY_LAYOUT_ID, layoutId);
        bundle.putBoolean(QRCode.KEY_IS_REPEATED, isRepeated);
        bundle.putLong(QRCode.KEY_SCAN_INTERVAL, scanInterval);
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = null;
        if (bundle != null) {
            int layoutId = bundle.getInt(QRCode.KEY_LAYOUT_ID);
            if (layoutId != -1) {
                view = inflater.inflate(layoutId, null);
            }
            mIsRepeated = bundle.getBoolean(QRCode.KEY_IS_REPEATED);
            mScanInterval = bundle.getLong(QRCode.KEY_SCAN_INTERVAL);
        }

        if (view == null) {
            view = inflater.inflate(R.layout.qrcode_fragment_capture, null);
        }
